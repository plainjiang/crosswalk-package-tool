...
DebProject.prototype.setChangeLog = function (version, hash, changelog, path) {
    Shell.pushd(path);
    var logs = Shell.exec(['git log --oneline --no-merges', 'HEAD...'+hash].join(' '), {silent:true}).output.split('\n');
    Shell.popd();
    ...
    for (var i = 0; i < logs.length; i++) {
        if (logs[i].length == 0)
            continue;
	try {
        cmd = ['dch', item_option, '-c', changelog, '"'+logs[i].replace(/"/g, '\\"').replace(/[0-9a-z]+ /, '') +'"'].join(' ');
        //Shell.exec(['dch', item_option, '-c', changelog, '"'+logs[i].replace(/[0-9a-z]+ /, '')+'"'].join(' '));
        Shell.exec(cmd);
	} catch(e) {
		console.log(e);
	}
...
function(deb_version_, callback) {
    ...
    var cmd = ['tar zcf', target,
                "--exclude-vcs --exclude=android_tools --exclude=LayoutTests --exclude=src/out --exclude=src/win8 --exclude=src/v8/test",
               "--exclude='*.pyc'", '--transform="s:^src:crosswalk-' + version_str + '-' + deb_version + ':S"', 'src/'].join(' ');
    Console.log(cmd);
    Shell.exec(cmd);
